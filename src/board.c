...
static inline unsigned long long get_stable_edge(const unsigned long long P, const unsigned long long O)
{
	...
	return edge_stability[P & 0xff][O & 0xff]
	    ...
	    |  A1_A8[edge_stability[((P & 0x0101010101010101ULL) * 0x0102040810204080ULL) >> 56][((O & 0x0101010101010101ULL) * 0x0102040810204080ULL) >> 56]]
	    |  H1_H8[edge_stability[((P & 0x8080808080808080ULL) * 0x0002040810204081ULL) >> 56][((O & 0x8080808080808080ULL) * 0x0002040810204081ULL) >> 56]];
}
